...
const (
	...
	indexFile    = "README.md"
	indexHeader  = "<!-- Generated by `mise run generate:adr-index`. Do not edit by hand. -->"
)
...
// statusWord is the leading state of a Status line, which often continues into a
// sentence about what superseded it.
func statusWord(status string) string {
	for i, r := range status {
		if r == ' ' || r == ';' || r == ',' || r == '(' {
			return strings.ToLower(status[:i])
		}
	}
...
func renderIndex(adrs []ADR) string {
	adrs = append([]ADR(nil), adrs...)
	sortADRs(adrs)

	var b strings.Builder
	b.WriteString(indexHeader)
	b.WriteString("\n\n# Decisions\n\n")
	b.WriteString("Architecture and infrastructure decisions, oldest first.\n\n")
	b.WriteString("An ADR is identified by its filename — the date orders it, the slug names it.\n")
	b.WriteString("Prose cites the slug alone: `(ADR terminal-transport)`. There is no number to\n")
	b.WriteString("allocate, so two branches can add an ADR without colliding. Add one with\n")
	b.WriteString("`mise run adr:new -- \"Title of the decision\"`.\n\n")
	b.WriteString("| Date | ADR | Decision | Status |\n")
	b.WriteString("| ---- | --- | -------- | ------ |\n")
	for _, a := range adrs {
		fmt.Fprintf(&b, "| %s | [`%s`](%s) | %s | %s |\n",
			a.Date, a.Slug, a.Name, escapePipes(a.Title), statusWord(a.Status))
	}
	return b.String()
}
