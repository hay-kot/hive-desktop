...
# See your first items

The starter feeds you get for free, and the core loop — read, act, and let an agent configure the rest.
...
[Keyboard shortcuts](../configuration/keybindings.md) shows how to change them.
...
See [Actions](../inbox/actions.md) for the full model and the `actions.yml` schema.
...
`claude` CLI on your PATH. [Agent workspaces](../chats/agent-workspaces.md)
...
when matching items arrive. [Flows](../inbox/flows.md) builds one up node
by node.
