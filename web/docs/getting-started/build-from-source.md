...
Clone the repository, install the toolchain with mise, and build or run the app yourself. macOS today; Linux builds in a container.

The [install script](index.md#install) is the short path. This page is for building
...
- **tmux 3.2 or newer**, optional. The app runs without it; terminal mode and
  agent chats need it. See [Terminal mode](../code/terminal-mode.md).
...
- To replay first run without touching state, or work offline against
  ...
  `development` on the [settings reference](../configuration/settings.md#development).
...
[settings reference](../configuration/settings.md#where-the-files-live). Only
