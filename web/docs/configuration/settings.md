...
[Flows](../inbox/flows.md) and [Actions](../inbox/actions.md).
...
published it downloads and verifies the build for you, with no manual download
step. A build tracks the channel it was released on. There are three channels,
...
you to that channel's current version. **Settings ▸ About** shows the running
build and the last check.
...
[system permission](../getting-started/notifications.md).
...
combo syntax are on [Keyboard shortcuts](keybindings.md);
...
Where [agent workspaces](../chats/agent-workspaces.md) live. Configurable so
