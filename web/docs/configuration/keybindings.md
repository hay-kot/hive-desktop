...
**Text size** (`terminal.text-size-increase`, `-decrease`, `-reset`) steps through the **Settings ▸ Terminal** font sizes wherever a terminal is on screen (a Code session, Chats, or the pop-up panel) and saves the result to the same setting. On macOS these chords no longer zoom the window: **Zoom In**, **Zoom Out** and **Actual Size** stay in the **View** menu without a shortcut, because window zoom scales the window instead of reflowing the terminal grid.
