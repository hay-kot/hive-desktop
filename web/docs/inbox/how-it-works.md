...
Workspaces, flows, sources, feeds, actions, and notifications — the model behind the inbox, and where each piece lives on disk.
...
hot-reloaded, so an edit takes effect without a restart. [Flows](flows.md)
...
each needs to connect, is on [Sources and webhooks](sources.md).
...
macOS, banners require the [system permission](../getting-started/notifications.md).
...
- **Code** (<kbd>g</kbd> <kbd>c</kbd>) attaches to the tmux sessions your
  ...
  [Terminal mode](../code/terminal-mode.md).
...
[settings reference](../configuration/settings.md#where-the-files-live).
