...
// broker fans one session's events out to at most one subscriber, the same
// one-transport-per-session rule tmuxcc has.
//
// A slow subscriber is answered by coalescing rather than by dropping or
// pausing: consecutive Output for the same window is concatenated in place, so
// the queue stops growing while the bytes stay whole and in order. That is only
// sound because this backend owns the byte stream end to end — there is no
// frame boundary in a PTY read that means anything to the far end.
type broker struct {
	mu       sync.Mutex
	sub      *subscriber
	maxBytes int
...
func (b *broker) Publish(ev Event) {
	...
	if out, ok := ev.(Output); ok {
		if n := len(sub.queue); n > 0 {
			if last, isOutput := sub.queue[n-1].(Output); isOutput && last.WindowID == out.WindowID {
