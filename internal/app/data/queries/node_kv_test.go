...
func TestNodeKV_Delete(t *testing.T) {
	...
	require.NoError(t, db.DeleteNodeKV(ctx, DeleteNodeKVParams{FlowID: "flow-1", NodeID: "dedup", Scope: KVScopeNode, Key: "seen"}))
	...
	require.NoError(t, db.DeleteNodeKV(ctx, DeleteNodeKVParams{FlowID: "flow-1", NodeID: "dedup", Scope: KVScopeNode, Key: "never"}))
}
