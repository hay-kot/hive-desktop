...
func TestRecoverInterruptedOutputCommands_JoinsTransaction(t *testing.T) {
	...
	command, created, err := db.ConfirmOutputCommand(ctx, "review", "item-1", []byte(`{}`), models.ItemRef{})
...
func TestConfirmOutputCommandDeduplicatesExistingCommand(t *testing.T) {
	...
	enqueueTestCommand(t, database, "action-a", "k1")
	row, created, err := database.ConfirmOutputCommand(ctx, "action-a", "k1", []byte(`{"v":2}`), models.ItemRef{})
	require.NoError(t, err)
	...
	existing, created, err := database.ConfirmOutputCommand(ctx, "action-a", "k1", []byte(`{"v":3}`), models.ItemRef{})
	require.NoError(t, err)
	...
	rerun, err := database.RerunOutputCommand(ctx, "action-a", "k1", []byte(`{"v":4}`), models.ItemRef{})
	require.NoError(t, err)
...
func TestRerunOutputCommandRequiresPriorRun(t *testing.T) {
	...
	_, err := database.RerunOutputCommand(t.Context(), "action-a", "missing", []byte(`{}`), models.ItemRef{})
...
func TestRerunOutputCommandRejectsActivePriorRun(t *testing.T) {
	...
	_, err := database.RerunOutputCommand(t.Context(), "action-a", "active", []byte(`{}`), models.ItemRef{})
