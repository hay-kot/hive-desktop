...
func (db *DB) InboxItemEvents(ctx context.Context, itemID int64, limit int) ([]InboxEventView, error) {
	...
	rows, err := db.ListInboxEventsByItem(ctx, ListInboxEventsByItemParams{ItemID: itemID, Limit: int64(limit)})
	if err != nil {
...
func (db *DB) MarkInboxItemsRead(ctx context.Context, profileID, feedID string) (int64, error) {
	...
	if feedID == "" {
		marked, err := db.MarkProfileInboxItemsRead(ctx, profileID)
		if err != nil {
			return 0, fmt.Errorf("marking every feed read for %q: %w", profileID, err)
		}
		return marked, nil
	}
	marked, err := db.MarkFeedInboxItemsRead(ctx, MarkFeedInboxItemsReadParams{ProfileID: profileID, FeedID: feedID})
	if err != nil {
...
func (db *DB) FeedCounts(ctx context.Context, profileID string) ([]FeedInboxCount, error) {
	rows, err := db.CountInboxItemsByFeed(ctx, profileID)
	if err != nil {
