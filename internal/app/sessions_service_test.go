...
func TestSessionsService_SessionLaunchOptions(t *testing.T) {
	...
	svc := newSessionsService(&fakeSessionLauncher{opts: expected}, manager, manager, &fakeSessionTmux{}, &fakeJobRunner{}, nil, nil, nil)
...
func TestSessionsService_CreateSessionValidatesBeforeTracking(t *testing.T) {
	...
	manager, _ := activeSession()
	svc := newSessionsService(launcher, manager, manager, &fakeSessionTmux{}, runner, nil, nil, nil)
...
func TestSessionsService_CreateSessionSurfacesDuplicateNameOnTheJob(t *testing.T) {
	runner := &fakeJobRunner{}
	manager, _ := activeSession()
	svc := newSessionsService(&fakeSessionLauncher{err: dispatch.ErrDuplicateSessionName}, manager, manager, &fakeSessionTmux{}, runner, nil, nil, nil)
...
func TestSessionsService_ListSessionsPassesEveryStateThrough(t *testing.T) {
	...
	svc := newSessionsService(&fakeSessionLauncher{}, manager, manager, &fakeSessionTmux{}, &fakeJobRunner{}, nil, nil, nil)
...
func TestSessionsService_RenameSessionRenamesTmuxBeforeTheStore(t *testing.T) {
	...
	svc := newSessionsService(&fakeSessionLauncher{}, manager, manager, tmux, &fakeJobRunner{}, nil, nil, nil)
...
func TestSessionsService_DeleteAndRecycleRunAsJobsLabelledWithTheSessionName(t *testing.T) {
	for _, tt := range []struct {
		...
		t.Run(tt.name, func(t *testing.T) {
			manager, _ := activeSession()
			runner := &fakeJobRunner{}
			svc := newSessionsService(&fakeSessionLauncher{}, manager, manager, &fakeSessionTmux{}, runner, nil, nil, nil)
...
func TestSessionsService_UnavailableWithoutDependencies(t *testing.T) {
	svc := newSessionsService(nil, nil, nil, nil, &fakeJobRunner{}, nil, nil, nil)
