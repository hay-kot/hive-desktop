...
func TestWatcher(t *testing.T) {
	...
	t.Run("RemovedWorkspaceDropsItsWatch", func(t *testing.T) {
		...
		require.NoError(t, os.MkdirAll(wsDir, 0o700))
		require.NoError(t, os.WriteFile(filepath.Join(wsDir, manifestFileName), []byte("version: 1\nname: Recreated\nagent: claude\nautonomy: ask\n"), 0o600))
		waitForWorkspaceChange(t, changed)
	})
	...
	t.Run("RenamedWorkspaceDirectoryDropsOldAddsNew", func(t *testing.T) {
		...
		require.NoError(t, os.WriteFile(filepath.Join(newDir, manifestFileName), []byte("version: 1\nname: Renamed\nagent: claude\nautonomy: ask\n"), 0o600))
		waitForWorkspaceChange(t, changed)
	})
