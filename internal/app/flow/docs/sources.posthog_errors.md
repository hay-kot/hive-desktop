...
The payload carries the issue's `title` (its exception type, falling back to its description), a `url` that deep-links to the issue in PostHog, a `state` of `active`, `resolved` or `suppressed`, the `occurrences`, `users` and `sessions` counts over the `date_from` window, and `firstSeen`/`lastSeen`/`library` for a `function` node to route on.
