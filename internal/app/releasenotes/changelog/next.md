...
- **A Tasks view over hive's `hc` issue tracker** — a tree and detail split
  with filters, status changes, and delete/prune, reached from a titlebar
  icon, the command palette, and `/tasks`. A poll picks up changes made
  outside the app, such as from the `hive hc` CLI.
