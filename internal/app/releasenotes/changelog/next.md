...
- One source message is split into per-entity feed items rather than one
  combined row.
- A chat workspace writes its own launch command instead of picking one of
  three autonomy postures. The command is a template with the workspace
  directory, the generated MCP config and the session id available to it, so
  any CLI can run in a workspace, not only the two Hive shipped flags for. The
  editor offers the postures as one-click presets, seeded from your `hive`
  agent profiles, and warns when a command skips permission prompts. Existing
  workspaces migrate to the command their posture already ran.
