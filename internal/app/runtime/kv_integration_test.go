...
func kvTestRunner(t *testing.T, db *store.DB, script string) *runtime.Runner {
	t.Helper()
...
func openKVTestDB(t *testing.T) *store.DB {
	t.Helper()
...
func notifyOutputs(batch store.CommitBatch) int {
	count := 0
	for _, out := range batch.Outputs {
		if out.Sink.Kind == store.SinkKindNotify {
			count++
		}
	}
	return count
...
func kvMsg(id string) store.Msg {
	m := store.Msg{ID: id, Key: "item-1", Payload: json.RawMessage(`{"title":"hi"}`)}
	m.SourceKind = "github"
...
func TestRun_KVSetThenThrowPersistsNothing(t *testing.T) {
	...
	batch, err := runner.Run(ctx, []store.Msg{kvMsg("1")})
	require.NoError(t, err)
	...
	after, err := dedup.Run(ctx, []store.Msg{kvMsg("2")})
	require.NoError(t, err)
...
func TestRun_DedupMemoryIsDurableAcrossRunners(t *testing.T) {
	...
	first, err := kvTestRunner(t, db, dedupScript).Run(ctx, []store.Msg{kvMsg("1")})
	require.NoError(t, err)
	require.NoError(t, db.CommitBatch(ctx, first))

	rebuilt := kvTestRunner(t, db, dedupScript)
	second, err := rebuilt.Run(ctx, []store.Msg{kvMsg("2")})
	require.NoError(t, err)
