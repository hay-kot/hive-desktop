...
- **Config vs data split.** User-editable config (flows, `actions.yml`,
  ...
  keybindings: {}
  terminal: {tmux_path: ""}         # absolute path; "" discovers tmux (ADR 0038)
...
| `HIVE_DESKTOP_TERMINAL_TMUX_PATH` | Absolute path to tmux, skipping discovery (ADR 0038); empty searches `$PATH` then the usual package-manager prefixes |
