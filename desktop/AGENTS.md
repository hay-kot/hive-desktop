...
- **Config vs data split.** User-editable config (flows, `actions.yml`,
  ...
  appearance: {theme: "", terminal_font_size: "", terminal_show_windows: true, terminal_pool_size: 3}   # terminal_font_size: small/medium/large/xl/xxl, "" = medium; terminal_show_windows lists every session's windows in the terminal sidebar; terminal_pool_size is how many sessions stay attached for instant switching (1-6, ADR 0041)
...
| `HIVE_DESKTOP_APPEARANCE_TERMINAL_POOL_SIZE` | Sessions the terminal view keeps attached for instant switching (1-6, ADR 0041); values outside the range read as the default, 3 |
