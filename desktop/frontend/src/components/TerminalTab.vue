...
<template>
  <!-- Hidden rather than unmounted: a Terminal binds to one element for its
       ...
       xterm only takes focus from a press inside its own screen element, which
       is a whole number of cells: the padding around it, and the part-cell
       remainder the grid cannot cover, would otherwise swallow a press meant
       for the pane. -->
  <div
    v-show="active"
