...
describe('byType', () => {
  it('discovers exactly the node types with app modules (index.ts)', () => {
    expect(Object.keys(byType).sort()).toEqual(['action', 'feed', 'function', 'github-filter', 'notify', 'sources.exec', 'sources.github', 'sources.grafana_alerts', 'sources.grafana_metrics', 'sources.webhook'])
  })
...
describe('palette', () => {
  it('groups every registered type into its declared category', () => {
    ...
    expect(palette.Sources.map((d) => d.type).sort()).toEqual(['sources.exec', 'sources.github', 'sources.grafana_alerts', 'sources.grafana_metrics', 'sources.webhook'])
