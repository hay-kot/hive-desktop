...
> and bootstraps the webview (ADR 0032). See
...
| A new **streaming endpoint** (WebSocket/SSE) | Data-plane mount — raw handler at its own prefix, REST control plane beside it | [Terminal sessions](#terminal-sessions), ADR 0032 |
...
internal/
  ...
  adapter/                        # driving adapters, all in-process
    ...
    httpapi/                      # REST + SSE, mounted via ServeHTTP at a Route.
                                  ...
                                  #   is a separate raw mount (ADR 0032)
...
them is the constraint (ADR 0032):
