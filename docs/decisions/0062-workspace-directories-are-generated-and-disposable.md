# 0062 — Workspace directories are generated and disposable
...
`internal/app/skills` (ADR 0033) already writes generated files into a
...
### 2. The explicit departure from ADR 0033

ADR 0033's skills installer hash-tracks its output and refuses to clobber a
...
Related decisions: ADR 0033 (the drift-tracked skill installer this
explicitly departs from, and why), ADR 0061 (the autonomy model that governs
what a workspace's agent is allowed to do once it is generated), ADR 0009
