# 0054 — Terminal mode is hidden on a trip to the hub, never unmounted
...
ADR 0042 stopped a session switch from detaching, so the pane no longer blanks
...
back paid, per pooled session, exactly what ADR 0042 exists to avoid — spawn
...
`PopupTerminal` already had the rule this needed (ADR 0048): mounted on first
...
- Once the mode has been opened, the app holds up to `terminal_pool_size` tmux
  ...
  overflow bound at bay. This extends ADR 0042's consequence, it does not
