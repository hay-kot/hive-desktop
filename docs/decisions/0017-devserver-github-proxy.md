...
9. **One proxy, shared by every worktree.** The response cache is already cross-process (WAL, `busy_timeout`), so a second proxy would not corrupt anything — but `singleflight` collapsing and the overlay store are both per-process. Two proxies would mean the dashboard being clicked might not control the instance being watched, which is a confusing failure that costs more than the sharing is worth.
   ...
   The singleton is made robust rather than assumed. devserver probes `/_ctl/health` on startup and exits 0 when one of its own is already listening, so `mise run devserver` is safe to run from any worktree without checking first; it binds before announcing so the probe's race is closed by the bind itself; and a foreign process on the port is fatal rather than silently tolerated.
...
10. **Every development run is proxied by default.** `cmd/devtools prepare` writes the API base into the worktree's `launch.env`, so opting in costs nothing and the shared budget is the default rather than the thing someone remembers. Opting out is setting the same variable empty in the gitignored `overrides.env`, which mise loads second.
    ...
    Defaulting to the proxy makes a missing proxy the common failure, so `desktop:dev` preflights it (`devtools check-proxy`) and fails with both remedies spelled out. The preflight reads the effective environment, so opting out disables it too. The alternative — letting the app start and having every GitHub call fail with connection-refused — moves the same problem somewhere much harder to read.
...
- Development now depends on a running devserver by default. That is the point — it is what makes the shared budget automatic rather than opt-in — but it means one more process to have running, and the preflight exists so forgetting it is a clear message instead of a wall of failed GitHub calls.
