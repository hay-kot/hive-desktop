# 0066 — `ptyterm` terminals are caller-addressed and concurrency-capped
...
ADR 0048 gave `ptyterm` an id-keyed set of terminals, but `Open` (`manager.go`)
...
5. **The stream is renamed, not duplicated.** `PopupTerminalStreamHandler`
   ...
   prefix cover it without a second rule (ADR 0048 point 4). It is a rename
...
6. **The pop-up keeps minting.** `PopupTerminalsService.Open` still passes no
   `ID` — nothing about a pop-up wanted to be addressable, and ADR 0048's rule
...
### What still holds from ADR 0048
...
ADR 0048 point 2 said a terminal "is addressed by an id this process mints …
...
rather than minting over it. ADR 0048's "what is not built" list already noted
