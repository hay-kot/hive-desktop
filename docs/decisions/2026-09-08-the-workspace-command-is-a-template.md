...
the only source of the launch line. `autonomy:` is deleted, migrated forward
at manifest version 4.
...
### 2. `agent:` is a label, not a gate

It selects the icon, the activity classifier, and the resume probe. An agent
key this build has never heard of is a normal workspace. `ErrUnknownAgent`,
`ErrNoAutonomyMapping`, `ErrPostureUnavailable` and
`ErrCommandNotASingleWord` are all deleted.

The generator writes **every** known MCP config format into every workspace
regardless of the agent named, so a template can point an unknown CLI at
whichever format it reads. That is what makes an unmapped agent a real
workspace rather than a degraded one.

### 3. hive's config seeds presets and nothing else
...
### 4. Danger is derived, never declared
...
### 5. The template is validated where it is written
...
### 6. Resume is a property of the template
...
(the tmux session a rendered line runs in), ADR [yaml-config-migration](2026-07-24-yaml-config-migration.md)
(the forward-only migration chain the version 4 step joins).
