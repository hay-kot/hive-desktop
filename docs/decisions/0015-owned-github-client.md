# 0015 — The desktop owns its GitHub client
...
needs, consumed here read-only under the vendoring policy (ADR 0002). An
...
ADR 0013 had already forked the credential half of the integration,
...
there is nothing left to seam. `token.go` is not ported (dead by ADR 0013).
...
- ADR 0002's scope narrows in practice: vendoring remains the policy for
  hive's session/messaging/store internals; the GitHub integration is now
  desktop-owned end to end (credentials by ADR 0013, client by this ADR).
