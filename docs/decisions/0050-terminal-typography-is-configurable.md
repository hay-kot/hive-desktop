...
from them correctly. What changed was the *rasterizer*: ADR 0038 moved every
...
- **The preload got wider.** `loadTerminalFaces` now warms both weights *and*
  ...
  pane opened (ADR 0038) and the weights are no longer fixed at 400/700.
- **Verification stays visual.** As with ADR 0038, nothing in CI can see that
