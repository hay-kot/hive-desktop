# 0010 — goja for function nodes, behind a ScriptRuntime port
...
A `function` node runs user-authored code against every message that reaches it. Until now that code was JavaScript evaluated by `new Function(...)` inside a Web Worker, which meant flow execution needed a browser. Moving the engine into Go (ADR 0011) needs a script engine that runs in the same process.
