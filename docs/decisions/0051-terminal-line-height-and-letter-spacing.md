# 0051 — Terminal line height and letter spacing are settings, and line height defaults to 1.2

- **Status:** accepted (supersedes [0038](0038-terminal-atlas-renderer.md)
  decision 4)
- **Date:** 2026-07-31

## Context

ADR 0050 made family, size and weight configurable but left the cell metrics
...
ADR 0038 decision 4 recorded that pinning either was not a fix for the
...
handling, but it is already a degradation path (ADR 0038 consequence 4).
...
1. **`lineHeight` and `letterSpacing` are user settings**, persisted in
   `settings.yaml` and applied to open panes, alongside the ADR 0050 typography
...
3. **Letter spacing's default must stay 0.** Appearance settings encode "nothing
   persisted" as the zero value (ADR 0014), and 0 is also a meaningful setting
...
- **ADR 0038 decision 4 is half-retired.** Its first half stands — neither knob
...
- **Verification stays visual, and Settings ▸ Appearance is where it happens.**
  ...
  settings are for — Canvas2D's heavier rasterisation (ADR 0050) and
  cell-stroked box drawing (ADR 0038) — so the preview is an xterm instance or
...
- **One renderer claim serves all three surfaces.** Session panes, the pop-up
  ...
  renderer, which is the failure ADR 0038 exists to prevent.
