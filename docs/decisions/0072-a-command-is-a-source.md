# 0072 — A command is a source: `sources.exec` runs a CLI on the tick and ingests its stdout as a snapshot
...
The connector shape itself is settled (ADR 0012). What was not settled is the
...
The command runs `sh -c` with `execenv`'s resolved environment (ADR 0041, ADR
...
node, per the fan-out pattern of ADR 0035. That pattern carries three costs a
