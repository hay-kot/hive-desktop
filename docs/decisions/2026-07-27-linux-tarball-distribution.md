...
Linux needed to become a published target alongside macOS ([r2-manifest-distribution](2026-07-23-r2-manifest-distribution.md), [release-channels](2026-07-23-release-channels.md)). The wails scaffold already carried build assets for four formats — AppImage, `.deb`, `.rpm`, and an Arch package via nfpm — so the question was which of them to actually publish, and whether the in-app updater could keep working across them.
...
5. **The terminal installer supplies user-level desktop integration.** It checks the downloaded binary for missing runtime libraries, writes a `.desktop` entry and icon under the XDG data directory, and refreshes the desktop database when the tool is available. It reports the distribution-specific dependency command but does not run a package manager or `sudo` on the user's behalf.
...
- A raw tarball still has no launcher entry, icon registration, or managed dependency install. The terminal installer supplies the first two and reports an actionable command when GTK4 or WebKitGTK 6.0 is missing.
