...
The agent HTTP API (ADR 0021) had no way to describe itself. A consumer — the
...
"single declaration, many consumers" pattern (ADR 0009). Documentation should
...
- **A code-first OpenAPI library (kin-openapi, swaggest, huma).** Rejected: each
  ...
  require rewriting handlers off the ADR 0022 errchain shape.
