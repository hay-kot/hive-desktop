...
compatibility: Requires a desktop instance built from this worktree with the agent API enabled (the default in every prepared worktree — launch.env sets HIVE_DESKTOP_WEBHOOKS_ENABLED and HIVE_DESKTOP_DEVELOPMENT_API_ENABLED). curl and jq.
---

# Observe the pipeline through the agent API

The desktop exposes a loopback read + reload API over `app.App` (ADR 0019), so a
test asserts against a supported surface instead of `desktop-pipeline.db`. It
rides the **webhook listener's port** — one port serves both webhook push
(`/hooks/…`) and this API (`/api/…`).
...
The API shares the webhook port, written to the worktree's `launch.env`:

```bash
API=127.0.0.1:$(grep HIVE_DESKTOP_WEBHOOKS_PORT launch.env | cut -d'"' -f2)
curl -s $API/api/status | jq .   # {build, webhook:{running,host,port,pathPrefix}, api:{pathPrefix}}
curl -s $API/api/help | jq .     # endpoints + the reload->read->retry loop
```

If `/api/status` refuses the connection, the app is not running, was not built
from this worktree, or webhooks are off (the API needs the webhook listener). A
fresh `mise run desktop:dev:prepare` turns both on.
...
- **The API needs webhooks enabled.** It rides that listener; if webhooks are
  off there is no server to mount on.
